...
The script handles all common tasks: build, run, stop, restart, rebuild, logs, and cleanup.
...
2. Rebuild and restart:
   ...
   podman run -d --name reality-check -p 8080:80 reality-check:latest
   ```
3. If service worker is caching old files, click the **"reload"** link at the bottom of the app
...
- Use a higher port (8080) or enable rootless port binding (see Rootless Mode above)
...
Service worker is enabled for offline PWA functionality:
```bash
...
**Container Size:** The built image is approximately 25-30 MB (nginx:alpine base + PWA files).
...
See [LICENSE](./LICENSE) for full terms. Modified deployments must disclose source code.
